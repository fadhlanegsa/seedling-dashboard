    ...
    <div class="col-md-8">
        ...
        <div class="card mt-3" style="border-color: #e67e22;">
            ...
            <div class="card-body">
                <p class="text-muted">
                    <i class="fas fa-info-circle"></i>
                    Gunakan fitur ini jika pemohon <strong>tidak datang mengambil bibit</strong>. Stok akan dikembalikan ke persemaian dan pemohon akan mendapat notifikasi email.
                </p>
...
<div id="cancelModal" class="modal" style="display: none;">
    <div class="modal-content">
        <h3><i class="fas fa-ban"></i> Batalkan Permintaan</h3>
        <p class="text-muted">Pembatalan akan mengembalikan stok ke persemaian dan mengirim notifikasi ke pemohon.</p>
        <form id="cancelForm">
            <input type="hidden" name="<?= CSRF_TOKEN_NAME ?>" value="<?= generateCSRFToken() ?>">
            <input type="hidden" name="request_id" value="<?= $request['id'] ?? '' ?>">
            <div class="form-group">
...
document.getElementById('cancelForm').addEventListener('submit', function(e) {
    ...
    fetch('<?= url('bpdas/cancelRequest') ?>', { method: 'POST', body: formData })
    .then(r => r.json())
    .then(data => {
        if (data.success) {
            alert(data.message);
